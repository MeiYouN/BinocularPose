...
class StereoCamera(object):
    ...
    def capture2(self, left_video, right_video, save_dir):
        ...
        while True:
            ...
            if key == ord('q'):
                ...
                cv2.imwrite(os.path.join(save_dir + "/left", "{:0=6d}.jpg".format(i)), frameL)
                cv2.imwrite(os.path.join(save_dir + "/right", "{:0=6d}.jpg".format(i)), frameR)
                i += 1
    ...
    def capture1(self, video, save_dir):
        ...
        width, height, numFrames, fps = 2560, 720, -1, 30
        ...
        while True:
            ...
            if self.detect:
                ...
                l = frameL.copy()
                r = frameR.copy()
            ls = cv2.resize(l, (640, 480))
            rs = cv2.resize(r, (640, 480))
            cv2.imshow('left', ls)
            cv2.imshow('right', rs)
            key = cv2.waitKey(10)
            if key == ord('q'):
    ...
    def cap_one(self, left_video, save_dir):
        """
        用于采集双USB连接线的双目摄像头
        :param left_video:int or str,左路视频路径或者摄像头ID
        :param right_video:int or str,右视频路径或者摄像头ID
        ...
        :return:
        """
        self.create_file(save_dir)s
        cap = cv2.VideoCapture(left_video)
        widthL, heightL, numFramesL, fpsL = self.get_video_info(cap)
        print("capL:\n", widthL, heightL, numFramesL, fpsL)
        i = 0
        while True:
            isuccessL, frameL = cap.read()
            if not (isuccessL):
                print("No more frames")
                break
            if self.detect:
            ...
            if key == ord('q'):
                ...
                print("save image:{:0=3d}".format(i))
                cv2.imwrite(os.path.join(save_dir, "{:0=6d}.jpg".format(i)), frameL)
                i += 1
        cap.release()
        cv2.destroyAllWindows()
...
def get_parser():
    width = 9
    height = 6
    left_video = 'C:\\Users\Zhang55\Desktop\EveryThing\EasyMocap\mydata\data10\cal\intri\\02.mp4'
    right_video = ''
    save_dir = "C:\\Users\Zhang55\Desktop\EveryThing\EasyMocap\mydata\data10\cal\intri\images\\02"
    parser = argparse.ArgumentParser(description='Camera calibration')
    parser.add_argument('--detect', type=bool, default=False, help='detect corner')
    ...
    parser.add_argument('--left_video', type=str, default=left_video, help='left video file or camera ID')
    parser.add_argument('--right_video', type=str, default=right_video, help='right video file or camera ID')
    parser.add_argument('--save_dir', type=str, default=save_dir, help='YML file to save calibrate matrices')
    parser.add_argument('--type', type=int, default=1, help='s')
    return parser
...
if __name__ == '__main__':
    ...
    if args.type == 2:
        ...
        stereo.cap_one(left_video=args.left_video, save_dir=args.save_dir)
